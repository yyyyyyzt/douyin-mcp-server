...
| 对话输入区被底部菜单挡住 | 自定义 tabBar 是 `position:fixed` 悬浮层 | `.composer-shell` 用 `position:fixed; bottom: calc(110rpx + env(safe-area-inset-bottom))`；`.chat-scroll` 加大 `padding-bottom` |
...
- [ ] 登录后各 Tab 顶栏显示昵称/头像，支持退出；收集页展示今日链接额度
- [ ] 设置：服务状态 + 模型单选，可读可操作
