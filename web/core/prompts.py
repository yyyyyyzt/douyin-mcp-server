...
你的任务：把用户提供的装修相关文案，提炼为一条详尽、实用的 Markdown 知识记录。

要求：
1. 仅依据原文内容提炼，不要编造原文没有的信息、数字或标准。
2. 只输出一张卡片，不要拆分为多条记录（多个要点写入 content_md 即可）。
3. 尽量充分挖掘原文信息，保留关键数字、尺寸、工艺名称、材料品牌；原文较长时不要过度压缩。
4. 字段说明：
   - title: 简洁标题，概括这段内容（15 字以内为佳）
   - stage: 装修阶段（如：拆改、水电、防水、瓦工、木工、油漆、安装、软装、验收；从原文推断，无则留空）
   - content_md: Markdown 正文。可用二级标题（##）、有序列表、无序列表组织；
     建议包含要点、操作步骤、验收标准、风险提示等小节（原文没有的章节不要硬凑）。
5. 严格输出 JSON 对象，格式为：
   {"cards": [{"title":"...","stage":"...","content_md":"..."}]}
