...
> 状态：**P0~P3 已实施**（2026-07）；P4 真机矩阵待手动验收。
> 依据：[`FRONTEND_REFACTOR_PLAN.md`](FRONTEND_REFACTOR_PLAN.md)、
...
| 阶段 | 内容 | 状态 |
|---|---|---|
| P0 | 去掉 `100vh`；问答 flex 布局 + 底栏；知识库 `scroll-view` | ✅ |
| P1 | 统一 `page-root` 骨架；页面标题区 + 设置入口 | ✅ |
| P2 | `tdesign-miniprogram` + `custom-tab-bar` + `t-tab-bar`（fixed/placeholder/safe-area） | ✅ |
...
后期「报价单 / 装修进度 / 记账 / 预算管控」等能力**统一在问答 Tab 以对话形式呈现**，避免再开独立 Tab。

### 场景模型（`miniprogram/utils/chat-scenarios.js`）

| `SCENARIO_KINDS` | 含义 | 当前状态 |
|---|---|---|
| `knowledge_qa` | 基于个人知识库问答 | ✅ 已接通 `/api/chat` |
| `quote_review` | 结合报价单/合同审查 | ✅ 上传附件 + `/api/chat` |
| `progress_track` | 施工进度对话更新 | ⬜ UI 占位「即将上线」 |
| `expense_log` | 装修记账 | ⬜ UI 占位「即将上线」 |
| `budget_guard` | 预算管控 | ⬜ UI 占位「即将上线」 |

### 消息结构（`components/chat-message` 按 `kind` 分支渲染）

```js
{
  id, role: 'user' | 'assistant',
  kind: 'knowledge_qa' | 'quote_review' | 'progress_track' | ...,
  content: string,
  attachment?: { kind: 'document' | 'receipt' | 'progress_snapshot', filename, text? },
  grounded?, citations?, hasDocument?, pending?, error?
}
```

### 后期扩展步骤（无需改 Tab 架构）

1. 在 `QUICK_SCENARIOS` 将对应项 `enabled: true`
2. 在 `chat-scenarios.js` 增加 `buildChatRequestBody` 分支或新 API 路径
3. 在 `components/chat-message` 增加 `kind` 专属 UI（如记账卡片、预算进度条）
4. 后端新增接口（如 `/api/expenses`、`/api/budget`）返回结构化数据，由消息 `meta.widgets` 渲染

### 当前无法在本期完成、需后续改造的事项

| 项 | 原因 | 建议 |
|---|---|---|
| 进度 / 记账 / 预算的真实对话闭环 | 后端尚无对应 API 与数据模型 | 先定 schema（`renovation_projects`、`expenses`、`budgets`），问答 Tab 只扩消息类型 |
| 附件类型 `receipt` / `progress_snapshot` | 仅定义 `ATTACHMENT_KINDS`，未接上传与解析 | 复用 `chooseMessageFile` + 新 parse 接口 |
| 多轮上下文 / 会话历史持久化 | 当前 `messages` 仅页面内存 | 后期 `sessions` 表 + `GET /api/chat/history` |
| 流式输出（打字机效果） | `/api/chat` 非 SSE | 可选 WebSocket 或 chunked 响应 |
| 鸿蒙 Skyline `env(safe-area-inset-bottom)` | 已知平台限制 | 参考 TDesign [#4304](https://github.com/Tencent/tdesign-miniprogram/pull/4304) 用 `wx.getWindowInfo()` 注入 CSS 变量 |
| 问答输入栏键盘顶起细调 | 依赖真机 `adjust-position` / `cursor-spacing` | P4 真机验收后微调 `composer-bar` 高度 |
...
| 阶段 | 内容 | 状态 |
|---|---|---|
| **P0** | 去掉 Tab 页 `100vh`；问答页 scroll + 底栏；知识库列表 `scroll-view` | ✅ |
| **P1** | 统一 `page-root`；设置入口 | ✅ |
| **P2** | TDesign + `custom-tab-bar` | ✅ |
| **P3** | Popup / Toast / 组件替换 | ✅ |
| **P4** | 真机矩阵验收 | ⬜ |
